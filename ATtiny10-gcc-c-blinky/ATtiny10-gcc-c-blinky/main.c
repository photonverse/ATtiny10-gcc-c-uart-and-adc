...
ISR(TIM0_COMPA_vect)
{
	...
	switch (bitPos)
	{
		case 1: // Start bit in UART
			PORTB &= ~(_BV(PIN_UART));
			bitPos++;
			break;
		
		case 2 ... 9: // UART 8-bit data
			PORTB = (byteInBits & 1) ?
					PORTB | (_BV(PIN_UART)) :
					PORTB & (~(_BV(PIN_UART)));
			byteInBits >>= 1;
		...
		case 10:	// Stop bit without any parity bit in UART
			PORTB |= _BV(PIN_UART);
...
int main(void)
{
	...
	*/
	...
	ADC.
	*/
	ADCSRA	= _BV(ADEN) | (0b111 << ADPS0);	// ADEN: ADC Enable, (ADPS1 | ADPS0): ADC Prescaler Select = Division Factor 128 (0b111)
											// 8MHz / 128 = 0.016 msec. That is, if we take a delay of 1 ms to print out ADC, sampling is still 1.6%
